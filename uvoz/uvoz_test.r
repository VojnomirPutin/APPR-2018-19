...
#7. tabela: povprečne mesečne plače

mesecne.place.regije <- read.csv2(file = 'podatki/povprecne_mesecne_place.csv', header= FALSE, skip = 2, sep = ';', dec = '.', fill = TRUE,
                                            strip.white = TRUE, fileEncoding = 'Windows-1250', nrows = 405, blank.lines.skip = TRUE, skipNul = FALSE, na = c('-', '', ' '),
                                  col.names = c('Regija', 'Leto', 'a', 'Neto_placa'))
mesecne.place.regije <- mesecne.place.regije%>%fill(1:2)%>%filter(Neto_placa != '')
mesecne.place.regije <- mesecne.place.regije[,-3]
mesecne.place.regije$Regija <- gsub('^Posavska.*', 'Spodnjeposavska', mesecne.place.regije$Regija)
mesecne.place.regije$Regija <- gsub('^Primorsko-notranjska.*', 'Notranjsko-kraška', mesecne.place.regije$Regija)
mesecne.place.regije <- mesecne.place.regije%>%filter(Regija != 'SLOVENIJA', Regija != 'Vzhodna Slovenija', Regija != 'Zahodna Slovenija')
